...
export default function TasksScreen() {
  ...
  const todayLabel = useMemo(() => {
    ...
    return `${t('todos.todayPrefix')} ${formatted}`;
  }, [t]);
        ...
        <View className="flex-row items-center justify-between mt-1">
          <View>
            ...
            <Text className="text-[12px] text-blue-100">
              {doneCount} {t('todos.doneOf')} {total}
            </Text>
          </View>
        </View>
      </View>
      ...
      >
        <View className="flex-1 bg-black/40 items-center justify-center px-6">
          <View className={`w-full rounded-2xl  ${ themeName === 'light' ? 'bg-white p-5' : 'bg-slate-800 p-5' }`}>
            ...
            />
            <Text className={`text-[13px] ${ themeName === 'light' ? 'text-slate-700 mb-1' : 'text-slate-200 mb-1' }`}>{t('todos.NoteTime')}</Text>
            <TextInput
            ...
            <View className="flex-row justify-between">
              ...
              <View className="flex-row">
                ...
                >
                  <Text className={`text-[13px]  ${ themeName === 'light' ? 'text-slate-700' : 'text-slate-200' }`}>{t('todos.Cancel')}</Text>
                </Pressable>
                <Pressable
